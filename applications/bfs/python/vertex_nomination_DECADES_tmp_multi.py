...
@njit(int64[:](DecSparseGraphSpec(), int64[:], int64[:], int32, int32), nogil=True, pipeline_class=DEC_Pipeline)
def multi_bfs(graph, roots, next_layer, tid, num_threads):
    ...
    while len(top_layer) > 0:
        for each_node in top_layer:
            for i in range(graph.indptr[each_node]+tid, graph.indptr[each_node + 1], num_threads):
                e = graph.indices[i]
...
def vertex_nomination__kernel__(G, seeds, ret_top_nominee, ret_top_score, tid, num_threads):
    if tid == 0:
        ...
        bfs_results = multi_bfs(G, seeds, tid, num_threads)
