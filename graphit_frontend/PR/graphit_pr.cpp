...
int main(int argc, char * argv[])
{
  ...
  for ( int trail = (0) ; trail < (10) ; trail++ )
  {
    ...
    for ( int i = (1) ; i < (11) ; i++ )
    {
      ...
      if ((i) == ((1) ))
       { 
        frontier = builtin_const_vertexset_filter <updateVertexFirstRound>(updateVertexFirstRound(), builtin_getVertices(edges) );
       } 
      else
       { 
        frontier = builtin_const_vertexset_filter <updateVertex>(updateVertex(), builtin_getVertices(edges) );
